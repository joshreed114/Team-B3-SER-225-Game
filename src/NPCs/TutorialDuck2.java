...
public class TutorialDuck2 extends NPC {

	public TutorialDuck2(Point location, Map map) {
	...
	protected SpriteFont createMessage() {
		return new SpriteFont("Thats a platform, but its not working so use the path I made!", getX(), getY() - 10, "Arial", 12, Color.BLACK);
	}
	...
	public void drawMessage(GraphicsHandler graphicsHandler) {
		...
		graphicsHandler.drawFilledRectangleWithBorder(Math.round(getCalibratedXLocation() - 2),
				Math.round(getCalibratedYLocation() - 24), 335, 25, Color.WHITE, Color.BLACK, 2);
